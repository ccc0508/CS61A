...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    if m[1][1] * total_mass(m[1][2]) == m[2][1] * total_mass(m[2][2]):
        if not is_mobile(m[2][2]) and not is_mobile(m[1][2]):
            return True
        elif is_mobile(m[2][2]) and is_mobile(m[1][2]):
            if balanced(m[2][2]) and balanced(m[1][2]):
                return True
        else:
            if is_mobile(m[2][2]):
                if balanced(m[2][2]):
