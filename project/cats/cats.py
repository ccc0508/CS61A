...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    ps1 = [x for x in paragraphs if select(x)]
    if k + 1  > len(ps1):
        return ''
    return ps1[k]
    # END PROBLEM 1
...
def accuracy(typed, source):
    ...
    for i in range(len(typed_words)):  
        if i > len(source_words) - 1:
            continue
        if typed_words[i] == source_words[i]:
            source_num += 1
    if typed == source == "" or typed == source == '':
        return 100.0
    if source_num == 0:
        return 0.0
    return source_num / typed_num * 100
    # END PROBLEM 3
...
def feline_fixes(typed, source, limit):
    ...
    def select(a,b,n):
        ...
        if a[0] == b[0]:
            return select(a[1:],b[1:],n)
        else:
            return select(a[1:],b[1:],n + 1) + 1
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        ...
        for j in range(len(timestamps_per_player[i]) - 1):
            time = time + [timestamps_per_player[i][j + 1] - timestamps_per_player[i][j]]
        times = times + [time]
